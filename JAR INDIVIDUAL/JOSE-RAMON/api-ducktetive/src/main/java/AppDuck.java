        ...
        if (usuarios.size() > 0) {
            ...
            if (usuarios.get(0).getFkCargo() != 3) {
                ...
                do {
                    ...
                    switch (opcaoAdm) {
                        case 0:
                            List<Inovacao> inovacaos = con.query("SELECT Usuario.nome, Usuario.sobrenome, Usuario.telefone, Usuario.email, Empresa.razao_social AS nomeEmpresa, cargo.nome AS nomeCargo\n" +
                                    "FROM Usuario\n" +
                                    "JOIN Empresa ON Usuario.fkEmpresa = Empresa.idEmpresa\n" +
                                    "JOIN cargo ON Usuario.fkCargo = cargo.idCargo\n" +
                                    "WHERE Empresa.idEmpresa = ?;", new BeanPropertyRowMapper<>(Inovacao.class), usuarios.get(0).getFkEmpresa());
                            System.out.println(inovacaos);
                            break;
                        case 1:
                            log.gravar("O metodo logar() caiu no caso 1 na linha 282", "system");
                            String serial2 = null;
                            try {
                                for (Disco disco : looca.getGrupoDeDiscos().getDiscos()) {
                                    if (disco.getBytesDeEscritas() != null) {
                                ...
                                log.gravar("Exito ao executar o for na linha 290", "system");
                            }catch (ArithmeticException e){
                                log.gravar("Erro ao executar o for na linha 292", "erro");
                            }
